      ...
      {minimized ? (
        ...
        <div className="p-1 sm:p-2">
          <div className="flex justify-between items-center mb-1">
            <h2 className="text-sm sm:text-base font-bold text-gray-800">Upcoming Events</h2>
            <div className="flex gap-2">
              <button onClick={() => setMinimized(true)} className="text-gray-500 hover:text-gray-700 mr-1">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {events.map((event, index) => (
                  <tr key={index} className={index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                    ...
                    <td className="px-0 py-1 whitespace-nowrap text-gray-600 align-top text-[9px] sm:text-xs w-[45px] sm:w-[60px] truncate" title={event.classInfo}>{event.classInfo}</td>
                  </tr>
                ))}
              </tbody>
            </table>
